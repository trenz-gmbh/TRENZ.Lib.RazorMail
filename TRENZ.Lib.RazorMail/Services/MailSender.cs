...
using EL.Lib.RazorMail.SystemNetMailExtensions;
using MailKit.Net.Smtp;
using MimeKit;
using NLog;

namespace EL.Lib.RazorMail.Services
{
    public class MailSender
    {
        ...
        public async Task SendViaMailKitAsync(SmtpAccount account)
        {
            using (var client = new SmtpClient())
            {
                ...
                foreach (var item in Attachments)
                {
                    var attachment = bodyBuilder.Attachments.Add(item.Filename, item.FileData, ContentType.Parse(item.ContentType));
                ...
                _Log.Info($"Sending mail from {mail.From} to {string.Join(", ", mail.To)}, cc {string.Join(", ", mail.Cc)}, bcc {string.Join(", ", mail.Bcc)}");
